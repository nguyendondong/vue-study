...
const HandleRegister = async () => {
  try {
    await userState.register(name.value, email.value, password.value, confirm_password.value)
  } catch (error: any) {
    if (error.status === 403) {
      errors.value.message = error.data.errors
    } else {
      errors.value = typeof (error.data.errors) === 'object' ? error.data.errors : error.data.message
    }
  }
}
</script>
    ...
    <div class="min-h-screen flex flex-col items-center justify-center">
      ...
      >
        <div class="md:max-w-md w-full sm:px-6 py-4">
          <form @submit.prevent="HandleRegister">
            ...
            <div class="mt-8">
              ...
              <p v-if="errors.confirm_password" class="text-red-500 text-xs pt-1">
            ...
            <div class="mt-12">
              <button
                type="submit"
                class="w-full shadow-xl py-2.5 px-4 text-sm font-semibold rounded-full text-white bg-red-600 hover:bg-red-700 focus:outline-none"
              >
                Sign in
              </button>
            </div>
        ...
        <div class="md:h-full max-md:mt-10 bg-[#000842] rounded-xl lg:p-12 p-8">
          <img src="/signin-image.webp" class="w-full h-full object-contain" alt="login-image" />
        </div>
      </div>
    </div>
  </div>
